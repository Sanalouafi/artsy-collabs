<x-admin>
    <div class="bg-dark rounded-3xl pt-10 mb-5">
        <h1 class="text-white text-3xl font-bold mb-10 pt-5 text-center">Edit project</h1>
            ...
            <div class="text-gray-900 font-medium text-xs text-center flex flex-col items-center justify-center">
                <label for="image-input" class="cursor-pointer">
                    <img id="preview-image" class="h-40 w-40 rounded-full shadow-xl border-2 border-gray-400"
                        src="{{ $project->getFirstMediaUrl('project') }}" alt="logo">
                </label>
                    ...
                    <div class="mb-1 p-2">
                        <select name="partner_id"
                            ...
                            @foreach ($partners as $partner)
                                <option value="{{ $partner->id }}" @if ($project->partner_id == $partner->id) selected @endif>
                                    {{ $partner->name }}</option>
                            @endforeach
                        </select>
